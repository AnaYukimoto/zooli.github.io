    ...
    if($_POST) {
        ...
        if ($senhaCliente == $ConfirmarsenhaCliente) {
            ...
            if ($valido) {
                ...
                $mostrarTutor = isset($_POST['campos-adicionaisTutor']) ? $_POST['campos-adicionaisTutor'] : "";
                $mostrarCuidador = isset($_POST['campos-adicionaisCuidador']) ? $_POST['campos-adicionaisCuidador'] : "";

                $sql = "INSERT INTO tb_cliente (NOME_CLIENTE, IDADE_CLIENTE, CPF_CLIENTE, FONE_CLIENTE, FONE_CLIENTE2, EMAIL_CLIENTE, SENHA_CLIENTE)
                                        VALUES ('$nomeCliente', '$idadeCliente', '$cpfCliente', '$telefoneCliente', '$telefoneCliente2', '$emailCliente', '$senhaHash') ";
                ...
                if ($conexao->query($sql) === TRUE) {
                    ...
                    if ($conexao->query($mostrarCuidador) === TRUE) {
                        ...
                        echo "<script>alert('Erro ao inserir os dados de Cuidador no banco de dados'); window.location.href='cadastroCliente.php';</script>";
                    }
                    ...
                    if ($conexao->query($mostrarTutor) === TRUE) {
                        echo "<script>alert('Dados inseridos com sucesso'); window.location.href='login.php';</script>";
                    } else {
                        echo "<script>alert('Erro ao inserir os dados de Tutor no banco de dados'); window.location.href='cadastroCliente.php';</script>";
                    }
